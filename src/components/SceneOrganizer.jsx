...
export default function SceneOrganizer({ talkText, lyrics, styleReference, talkMeta, restoreState, onStateChange }) {
  ...
  // How scene COUNT is decided: "song" = one per lyric section (talk compressed
  // to fit), "talk" = follow the talk's natural points (more scenes, looser sync).
  const [sceneBasis, setSceneBasis] = useState("song");
  ...
  // always wins. This is what makes Conference, Session, and the titled speaker
  // appear automatically the moment a talk is picked — no button needed.
  useEffect(() => {
    if (!talkMeta) return;
    setMeta((prev) => ({
  ...
  async function buildScenes() {
    ...
    try {
      ...
      if (storyboardAvailable()) {
        ...
        try {
          const sb = await generateStoryboard({ lyrics, styleReference, talkText: talkText || "", sceneBasis });
      ...
      const outlineData = await generateOutline({ lyrics, styleBible: bible, talkText: talkText || "", sceneBasis });
  ...
  async function autoGenerateAll(sceneList) {
    ...
    for (const scene of list) {
      ...
      try {
        ...
        if (!firstGenerated) firstGenerated = imageDataUrl;
      } catch (e) {
        failures.push(n);
      } finally {
    ...
    // After scenes, generate the intro/outro cards so they match the story's
    // established style. Intro references the first generated image; outro the
    // last. Only generate cards that don't already have an image.
    const firstImg = firstGenerated || prevImage;
    const lastImg = prevImage; // most recent generated image

    // Make sure the title/credits metadata is filled BEFORE building cards,
    // otherwise the card renders the placeholder word "Untitled".
    let cardMeta = meta;
    if (!cardMeta.songTitle) {
      setAutoGenStatus("Filling in title & credits…");
      const filled = await autofillMeta();
      if (filled) cardMeta = filled;
    }

    // If we still have no title, skip auto-card generation rather than produce
    // an "Untitled" card. The user can fill the title and generate manually.
    if (!cardMeta.songTitle) {
      setError('Cards not auto-generated: no song title found. Fill in "Song title" under the intro card, then click "Generate image here" for the intro/outro.');
      return;
    }
    ...
    try {
      if (!endcards.intro?.image) {
        setAutoGenRunning(true);
        setAutoGenStatus("Generating intro card…");
        await genCard("intro", lockReference ? firstImg : "", cardMeta);
      }
      if (!endcards.outro?.image) {
        setAutoGenRunning(true);
        setAutoGenStatus("Generating outro card…");
        await genCard("outro", lockReference ? lastImg : "", cardMeta);
      }
    } finally {
      setAutoGenRunning(false);
      setAutoGenStatus("");
  ...
  async function autofillMeta() {
    ...
    try {
      ...
      const known = talkMeta || {};
      let resolved;
      setMeta((prev) => {
        resolved = {
          // Song title isn't in the talk metadata, so AI/lyrics is the source.
          songTitle: prev.songTitle || m.songTitle || "",
          // For the rest, prefer what the user typed, then the known facts from
          // the picked talk, then the AI's best guess as a last resort.
          speaker: prev.speaker || known.speaker || m.speaker || "",
          conferenceMonthYear: prev.conferenceMonthYear || known.conferenceMonthYear || m.conferenceMonthYear || "",
          session: prev.session || known.session || m.session || "",
          scripture: prev.scripture || "",
        };
        return resolved;
      });
  ...
  function cardBackgroundPrompt(kind, metaOverride) {
    ...
    if (kind === "intro") {
      const creditParts = [
        ...
        m.conferenceMonthYear ? `Based on a message from ${m.conferenceMonthYear} General Conference` : "",
        m.session ? `${m.session}` : "",
      ...
      return [
        ...
        `Large title at top: ${q(m.songTitle || "Untitled")}.`,
        creditParts.length
    ...
    // outro
    const outroScripture = m.scripture
      ? `Near the bottom left, an italic scripture quote rendered exactly: ${q(m.scripture)}.`
      : "";
    return [
      ...
      `Large title at top: ${q(m.songTitle || "Untitled")}.`,
      m.speaker ? `Italic credit line below the title: ${q("A song inspired by " + m.speaker)}.` : "",
  ...
  async function genCard(kind, referenceImageB64 = "", metaOverride = null) {
    setError("");
    setCardBusy((p) => ({ ...p, [kind]: true }));
    try {
      const notes = (cardNotes[kind] || "").trim();
      const basePrompt = endcards[kind]?.prompt?.trim() || cardBackgroundPrompt(kind, metaOverride);
      const prompt = notes
      ...
      setEndcards((p) => ({
        ...p,
        [kind]: { ...p[kind], image: imageDataUrl, prompt: basePrompt },
      }));
      return imageDataUrl;
    } catch (e) {
  ...
  async function genImage(scene) {
    ...
    try {
      const notes = (editNotes[scene.sceneNumber] || "").trim();
      const basePrompt = (scene.imagePrompt || scene.description || "").trim();
      if (!basePrompt) {
        setError(`Scene ${scene.sceneNumber}: this scene has no image prompt yet. Add prompt text (or use "Revise scene text") before generating.`);
        return;
      }
      const prompt = notes
        ? `${basePrompt}\n\nIMPORTANT CORRECTIONS — apply these changes: ${notes}`
  ...
  return (
    <section className="panel">
      ...
      <div className="row">
        ...
        <label className="row" style={{ gap: 6 }} title="Choose what sets the number of scenes: the song's sections, or the talk's natural points">
          <span className="note" style={{ margin: 0 }}>Scene count:</span>
          <select
            value={sceneBasis}
            onChange={(e) => setSceneBasis(e.target.value)}
            style={{ padding: "2px 6px" }}
          >
            <option value="song">Follow the song (one per section)</option>
            <option value="sections">More scenes (2+/verse, 2/chorus, 2/bridge)</option>
            <option value="talk">Follow the talk (more scenes, looser sync)</option>
          </select>
        </label>
      ...
      {styleBible && (
        <div className="style-bible">
          ...
          <dl>
            ...
            {styleBible.musicDirection && (
              <>
                <dt>Song genre</dt>
                <dd>{styleBible.musicDirection.genre}</dd>
                <dt>Song moods</dt>
                <dd>
                  {Array.isArray(styleBible.musicDirection.moods) && styleBible.musicDirection.moods.length > 0 ? (
                    <>
                      <div style={{ marginBottom: 6 }}>
                        {styleBible.musicDirection.moods.map((m, i) => (
                          <span key={i} className="lyric-tag" style={{ marginRight: 6 }}>{m}</span>
                        ))}
                      </div>
                      <button
                        className="btn btn-ghost"
                        ...
                        onClick={() => {
                          const txt = styleBible.musicDirection.moods.join(", ");
                          if (navigator.clipboard) navigator.clipboard.writeText(txt);
                          setCopiedScene("moods");
                          setTimeout(() => setCopiedScene((c) => (c === "moods" ? null : c)), 1500);
                        }}
                      >
                        {copiedScene === "moods" ? "Copied ✓" : "Copy moods"}
                      </button>
                    </>
                  ) : "—"}
                  {styleBible.musicDirection.notes && (
                    <div className="note" style={{ marginTop: 6 }}>{styleBible.musicDirection.notes}</div>
                  )}
                </dd>
              </>
            )}
      ...
      {scenes.length > 0 && (
        <EndCard
          ...
          notes={cardNotes.intro}
          onNotesChange={(v) => setCardNotes((p) => ({ ...p, intro: v }))}
      ...
      {scenes.length > 0 && (
        <EndCard
          ...
          notes={cardNotes.outro}
          onNotesChange={(v) => setCardNotes((p) => ({ ...p, outro: v }))}
  ...
  return (
    <div className="scene-card endcard">
      ...
      <label className="field" style={{ marginTop: 8 }}>
        <span className="lbl">
          Revision notes — used when you regenerate the card image
        </span>
        <input
          type="text"
          placeholder="e.g. 'warmer sky', 'less text at the bottom', 'mountains instead of city'"
          value={notes || ""}
