...
export default async (req) => {
  ...
  const system = [
    ...
    "{",
    '  "outline": [',
    '    { "sceneNumber": number, "imageIndex": number, "lyrics": string, "lyricSection": string, "beat": string }',
    "  ]",
    "}",
    "FIELD DEFINITIONS (important):",
    "- 'lyrics': the ACTUAL lyric words/lines this scene covers, copied verbatim",
    "  from the lyrics above (e.g. \"And she gathered grain in the fading light\").",
    "  This must be real lyric text, NOT a label.",
    "- 'lyricSection': a short LABEL for where it falls (e.g. \"Verse 1, lines 1-2\").",
    "- 'beat': a short phrase naming the visual moment.",
    "sceneNumber is sequential from 1 following the lyric order.",
  ].join("\n");
  ...
  const userContent =
    ...
    `by lyric order. For each scene, put the real lyric words in 'lyrics'.`;
