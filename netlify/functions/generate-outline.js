...
export default async (req) => {
  ...
  const system = [
    ...
    "}",
    "FIELD DEFINITIONS:",
    "- 'lyrics': the ACTUAL lyric words this scene covers, copied verbatim from",
    "  the lyrics above (real text, not a label).",
    "- 'lyricSection': a short LABEL (e.g. \"Chorus\" or \"Verse 2, lines 1-2\").",
    "- 'beat': a short phrase (under 12 words) naming the visual moment.",
    "Number scenes sequentially starting at 1, in song order.",
  ].join("\n");
  ...
  const userContent =
    ...
    `Return ONLY the outline as raw JSON. Put the real lyric words in 'lyrics' ` +
    `for each scene; keep each beat a short phrase.`;
