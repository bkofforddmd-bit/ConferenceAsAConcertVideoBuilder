...
export default async (req) => {
  ...
  const countRule = sceneBasis === "sections"
    ? [
        "- Use the SONG'S sections as MINIMUM scene counts, splitting each section's",
        "  lyric lines across its scenes so each scene covers DIFFERENT lines/moment:",
        "    Verse: at least 2. Pre-chorus: at least 1. Each Chorus instance: 2.",
        "    Bridge: at least 2. These are minimums — exceed them if content is rich.",
        "- Set each scene's 'lyrics' to the SPECIFIC lines it covers (subset of the",
        "  section); 'lyricSection' is the label (e.g. 'Verse 1', 'Chorus').",
      ]
    : sceneBasis === "talk"
    ? [
